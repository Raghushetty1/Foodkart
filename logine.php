...
<body>
    ...
    <div class="login" style=" margin:-5% 15%; padding:0% 26%; padding-left: 57%">
        <form method="post" style="  margin: 25% 23%" action="">
            <h2 syle="font-size:20px">LOG IN</h2>
            <label for uname>Employee Id</label>
            <input type="text" id="log" name="user" value="" placeholder="User Name"><br>
...
if(isset($_POST['submit']))
{
	...
	if($rslt->num_rows!=0)
	{
		$qry="select * from emp where user='$user' and pwd='$pwd'";
        $rslt=$cn->query($qry);
		if($rslt->num_rows!=0)
		{
            $_SESSION['user']=$user;
            echo"<script>alert('Loging in!');</script>";
            echo'<script>window.location.href="employee/emp.php?emp='.$user.'";</script>';
		}
		else
		{
