...
<body>
    ...
    <div style=" margin:-5% 15%; padding:0% 26%; padding-left: 57%" class="login">
        <form action=" " style=" margin:25% 23%" method="POST">
            <h2 syle="font-size:20px">LOG IN</h2>
            <label for uname>Admin Id</label>
            <input type="text" name="user" id="log" value="" placeholder="User Name"><br>

            <label>Password</label>
            <input type="password" name="pwd" id="password" value="" placeholder="Password"><br>
...
if(isset($_POST['submit']))
{
	...
	$qry="select * from admin where user='$user'";
	$rslt=$cn->query($qry);
	if($rslt->num_rows!=0)
	{
		$qry="select * from admin where user='$user' and pwd='$pwd'";
        $rslt=$cn->query($qry);
		if($rslt->num_rows!=0)
		{
            //echo $user;
	...
	{
        echo"<script>alert('Invalid login credintials!');</script>";
	}
	//echo "</font></form>";
	$cn->close();
}
